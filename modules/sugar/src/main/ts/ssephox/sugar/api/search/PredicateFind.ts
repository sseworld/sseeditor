...
const first: {
  <T extends Node = Node>(
    predicate: (e: SugarElement<Node>) => e is SugarElement<T>
  ): Optional<SugarElement<T & ChildNode>>;
  (predicate: (e: SugarElement<Node>) => boolean): Optional<
    SugarElement<Node & ChildNode>
  >;
...
const closest: {
  ...
  const is = (
    s: SugarElement<Node>,
    test: (e: SugarElement<Node>) => boolean
  ): s is SugarElement<Node> => test(s);
...
const sibling: {
  ...
  (
    scope: SugarElement<Node>,
    predicate: (e: SugarElement<Node>) => boolean
  ): Optional<SugarElement<Node & ChildNode>>;
} = (
  scope: SugarElement<Node>,
  predicate: (e: SugarElement<Node>) => boolean
): Optional<SugarElement<Node & ChildNode>> => {
  ...
  return child(
    ...
    (x) => !Compare.eq(scope, x) && predicate(x)
  );
};
