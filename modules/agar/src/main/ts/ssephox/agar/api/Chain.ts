...
import { Arr, Fun, Result } from "@ssephox/katamari";
...
const fromIsolatedChains = <T = any>(
  chains: Chain<any, any>[]
): Chain<T, T> => {
  const cs = Arr.map(chains, extract);
...
const fromIsolatedChainsWith = <T, U = any>(
  initial: T,
  chains: Chain<any, any>[]
): Chain<U, U> => fromIsolatedChains<U>([inject(initial)].concat(chains));
...
const fromPromise = <A, B>(f: (a: A) => Promise<B>): Chain<A, B> =>
