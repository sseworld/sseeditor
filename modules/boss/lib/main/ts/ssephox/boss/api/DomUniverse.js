...
import { Attribute, Compare, Css, Insert, InsertAll, PredicateFilter, PredicateFind, Remove, SelectorFilter, SelectorFind, SugarElement, SugarNode, SugarText, Traverse } from '@ssephox/sugar';
import TagBoundaries from '../common/TagBoundaries';
export default () => {
    const clone = (element) => {
        return SugarElement.fromDom(element.dom.cloneNode(false));
    };
    const document = (element) => Traverse.documentOrOwner(element).dom;
    ...
    const isNonEditable = (element) => SugarNode.isElement(element) && Attribute.get(element, 'contenteditable') === 'false';
    ...
    const getLanguage = (element) => SugarNode.isElement(element) ? Attribute.getOpt(element, 'lang') : Optional.none();
    return {
        up: Fun.constant({
