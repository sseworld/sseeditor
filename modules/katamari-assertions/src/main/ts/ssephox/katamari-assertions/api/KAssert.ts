...
export const eqOptional = <A>(
  message: TestLabel,
  expected: Optional<A>,
  actual: Optional<A>,
  testableA: Testable<A> = tAny
): void => Assert.eq(message, expected, actual, tOptional(testableA));
...
export const eqSome = <A>(
  ...
  actual: Optional<A>,
  testableA: Testable<A> = tAny
): void => eqOptional(message, Optional.some<A>(expected), actual, testableA);
...
): void => Assert.eq(message, expected, actual, tResult(testableA, testableE));
