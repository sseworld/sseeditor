...
export const repeatable = (delay: number): Repeatable => {
  ...
  const set = (functionToRepeat: () => void) => {
    ...
    revoke();
    intervalId.set(Optional.none<number>()); // Set to None initially
    const timeoutId = setInterval(functionToRepeat, delay) as unknown as number;
    intervalId.set(Optional.some(timeoutId)); // Set to Optional<number>
  };
