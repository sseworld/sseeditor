!(function (t) {
  ...
  (M.showHint = function (t, i, e) {
    ...
    M.defineExtension("showHint", function (t) {
      t = (function (t, i, e) {
        var n = t.options.hintOptions,
          o = {};
        for (var s in h) o[s] = h[s];
      ...
      if (!(1 < i.length)) {
        ...
        n.options.hint &&
          (M.signal(this, "startCompletion", this), n.update(!0));
      }
    }),
  ...
  function c(o, t) {
    ...
    var e = this,
      ...
      h = (this.hints = c.createElement("ul")),
      i = o.cm.options.theme;
    (h.className = "CodeMirror-hints " + i),
      (this.selectedHint = t.selectedHint || 0);
    for (var n = t.list, l = 0; l < n.length; ++l) {
      var a = h.appendChild(c.createElement("li")),
        u = n[l],
        f = "CodeMirror-hint" + (l != this.selectedHint ? "" : " " + F);
      null != u.className && (f = u.className + " " + f),
        (a.className = f),
        u.render
          ? u.render(a, t, u)
          : a.appendChild(c.createTextNode(u.displayText || N(u))),
        (a.hintId = l);
    }
    var p = s.cursorCoords(o.options.alignWithWord ? t.from : null),
      d = p.left,
      m = p.bottom,
      g = !0;
    (h.style.left = d + "px"), (h.style.top = m + "px");
    var v =
        r.innerWidth ||
    ...
    (o.options.container || c.body).appendChild(h);
    var w = h.getBoundingClientRect(),
      H = w.bottom - y,
      k = h.scrollHeight > h.clientHeight + 1,
      C = s.getScrollInfo();
    if (0 < H) {
      var b = w.bottom - w.top;
      if (0 < p.top - (p.bottom - w.top) - b)
        (h.style.top = (m = p.top - b) + "px"), (g = !1);
      else if (y < b) {
        (h.style.height = y - 5 + "px"),
          (h.style.top = (m = p.bottom - w.top) + "px");
        var A = s.getCursor();
        t.from.ch != A.ch &&
          ((p = s.cursorCoords(A)),
          (h.style.left = (d = p.left) + "px"),
    ...
    if (
      (0 < S &&
        (w.right - w.left > v &&
          ((h.style.width = v - 5 + "px"), (S -= w.right - w.left - v)),
        (h.style.left = (d = p.left - S) + "px")),
      k)
    )
      for (var T = h.firstChild; T; T = T.nextSibling)
        T.style.paddingRight = s.display.nativeBarWidth + "px";
    (s.addKeyMap(
      (this.keyMap = (function (t, n) {
        ...
        if (i) for (var c in i) i.hasOwnProperty(c) && e(c, i[c]);
    ...
    return (
      s.on(
        ...
        (this.onScroll = function () {
          ...
          if ((g || (n += h.offsetHeight), n <= i.top || n >= i.bottom))
            return o.close();
          (h.style.top = e + "px"), (h.style.left = d + C.left - t.left + "px");
        })
      ),
      M.on(h, "dblclick", function (t) {
        var i = E(h, t.target || t.srcElement);
        i && null != i.hintId && (e.changeActive(i.hintId), e.pick());
      }),
      M.on(h, "click", function (t) {
        var i = E(h, t.target || t.srcElement);
        i &&
          null != i.hintId &&
          (e.changeActive(i.hintId),
          o.options.completeOnSingleClick && e.pick());
      }),
      M.on(h, "mousedown", function () {
        setTimeout(function () {
          s.focus();
        }, 20);
      }),
      M.signal(
        ...
        "select",
        n[this.selectedHint],
        h.childNodes[this.selectedHint]
      ),
      !0
    );
  }
  ...
  (o.prototype = {
    close: function () {
      this.active() &&
        ...
        this.widget && this.data && M.signal(this.data, "close"),
    ...
    cursorActivity: function () {
      ...
      if (
        ...
        i.length - t.ch != this.startLen - this.startPos.ch ||
        t.ch < this.startPos.ch ||
        this.cm.somethingSelected() ||
        !t.ch ||
        this.options.closeCharacters.test(i.charAt(t.ch - 1))
      )
        this.close();
      else {
        var e = this;
    ...
    finishUpdate: function (t, i) {
      ...
      var e =
        ...
        (i && this.options.completeSingle);
      this.widget && this.widget.close(),
        (this.data = t) &&
          t.list.length &&
          (e && 1 == t.list.length
            ? this.pick(t, 0)
            : ((this.widget = new c(this, t)), M.signal(t, "shown")));
    },
  }),
    (c.prototype = {
      close: function () {
      ...
      changeActive: function (t, i) {
        if (
          (t >= this.data.list.length
            ? (t = i ? this.data.list.length - 1 : 0)
            : t < 0 && (t = i ? 0 : this.data.list.length - 1),
          this.selectedHint != t)
        ) {
          ...
          e && (e.className = e.className.replace(" " + F, "")),
            ...
            M.signal(this.data, "select", this.data.list[this.selectedHint], e);
        }
      },
    ...
    M.registerHelper("hint", "auto", {
      resolve: function (t, i) {
        ...
        return (e = t.getHelper(t.getCursor(), "hintWords"))
          ? function (t) {
              return M.hint.fromList(t, { words: e });
            }
          : M.hint.anyword
          ? function (t, i) {
              return M.hint.anyword(t, i);
            }
          : function () {};
      },
    }),
    M.registerHelper("hint", "fromList", function (t, i) {
      var e,
        n = t.getCursor(),
        o = t.getTokenAt(n),
        s = M.Pos(n.line, o.start),
        c = n;
      o.start < n.ch && /\w/.test(o.string.charAt(n.ch - o.start - 1))
        ? (e = o.string.substr(0, n.ch - o.start))
        : ((e = ""), (s = n));
      for (var r = [], h = 0; h < i.words.length; h++) {
        var l = i.words[h];
        l.slice(0, e.length) == e && r.push(l);
      }
      if (r.length) return { list: r, from: s, to: c };
    }),
