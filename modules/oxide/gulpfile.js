...
var exportLessVariablesToJson = new variablesOutput({
  filename: "build/skin-tool/less-variables.json",
});
...
gulp.task("copyFilesA", function () {
  return gulp
    .src(["./src/demo/styles.css", "./src/demo/scripts.js"])
    .pipe(gulp.dest("./build"));
});
...
gulp.task("copyFilesB", function () {
  return gulp
    .src([
      "./src/demo/skin-tool/*.less",
      "./src/demo/skin-tool/*.css",
      "./src/demo/skin-tool/*.js",
    ])
...
gulp.task("setupIconManager", function () {
  return gulp
    .src([
      "src/demo/iconManager.js",
      "./node_modules/@ephox/oxide-icons-*/dist/js/TinymceIcons.js",
    ])
...
gulp.task("buildSkinSwitcher", (done) => {
  ...
  const data = `uiSkins = ${JSON.stringify(
    uiSkins
  )}, contentSkins = ${JSON.stringify(contentSkins)}`;
...
gulp.task("serve", function () {
  ...
  gulp.watch(
    "./src/**/*.less",
    gulp.series("lint", "copyFilesC", "less", "minify-css")
  );
  gulp.watch(
    "./src/demo/**/*.html",
    gulp.series("buildHtml", "copyFiles", "buildSkinSwitcher")
  );
  gulp.watch(
    ["./src/demo/**/*.css", "./src/demo/**/*.js"],
    gulp.series("buildHtml", "copyFiles")
  );
...
const packages = ["@ephox/oxide-icons-material", "@ephox/oxide-icons-jam"];
gulp.task(
  "getInternal",
  shell.task(
    "npm install " +
      packages.join(" ") +
      " --no-save --registry http://nexus:8081/repository/npm-group/"
  )
);
