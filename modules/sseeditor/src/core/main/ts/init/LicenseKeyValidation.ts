...
export const validateEditorLicenseKey = (editor: Editor): void => {
  ...
  if (!hasApiKey && (Type.isUndefined(licenseKey) || validateLicenseKey(licenseKey) === 'INVALID')) {
    ...
    console.warn(`SSEEditor is running in evaluation mode. Provide a valid license key or add license_key: 'gpl' to the init config to agree to the open source license terms. Read more at https://www.tiny.cloud/license-key/`);
  }
};
