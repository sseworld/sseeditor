...
import { SSEEditor } from '../api/SSEEditor';
...
const makeStylesheetLoadingPromises = (editor: Editor, css: string[], framedFonts: string[]): Promise<unknown>[] => {
  const { pass: bundledCss, fail: normalCss } = Arr.partition(css, (name) => sseeditor.Resource.has(ContentCss.toContentSkinResourceName(name)));
  const bundledPromises = bundledCss.map((url) => {
    const css = sseeditor.Resource.get(ContentCss.toContentSkinResourceName(url));
