 ...
 *
 * @example
 * const parser = sseeditor.html.DomParser({ validate: true }, schema);
 * const rootNode = parser.parse('<h1>content</h1>');
 *
 * @class sseeditor.html.DomParser
 * @version 3.4
 */
...
const DomParser = (settings: DomParserSettings = {}, schema = Schema()): DomParser => {
   ...
   * @return {sseeditor.html.Node} Root node containing the tree.
   * @example
   * const rootNode = sseeditor.html.DomParser({...}).parse('<b>text</b>');
   */
