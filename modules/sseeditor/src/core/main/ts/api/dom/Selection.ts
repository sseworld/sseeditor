 ...
 * Consult the SSEEditor API Documentation for more details and examples on how to use this class.
 *
 * @class sseeditor.dom.Selection
 * @example
 * // Getting the currently selected node for the active editor
 * alert(sseeditor.activeEditor.selection.getNode().nodeName);
 */
 ...
 * @constructor
 * @method Selection
 * @param {sseeditor.dom.DOMUtils} dom DOMUtils object reference.
 * @param {Window} win Window to bind the selection object to.
 * @param {sseeditor.dom.Serializer} serializer DOM serialization class to use for getContent.
 * @param {sseeditor.Editor} editor Editor instance of the selection.
 */
const EditorSelection = (dom: DOMUtils, win: Window, serializer: DomSerializer, editor: Editor): EditorSelection => {
   ...
   * alert(sseeditor.activeEditor.selection.getContent({ format: 'text' }));
   */
   ...
   * sseeditor.activeEditor.selection.setContent('<strong>Some contents</strong>');
   */
   ...
   * const bm = sseeditor.activeEditor.selection.getBookmark();
   *
   * sseeditor.activeEditor.setContent(sseeditor.activeEditor.getContent() + 'Some new content');
   *
   * // Restore the selection bookmark
   * sseeditor.activeEditor.selection.moveToBookmark(bm);
   */
  ...
  const getRng = (): Range => {
    ...
    try {
      ...
      // IE throws unspecified error here if SSEEditor is placed in a frame/iframe
    }
   ...
   * sseeditor.activeEditor.selection.setNode(sseeditor.activeEditor.dom.create('img', { src: 'some.gif', title: 'some title' }));
   */
