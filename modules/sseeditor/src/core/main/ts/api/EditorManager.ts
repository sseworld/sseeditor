...
declare const window: Window & { sseeditor?: PreInit; tinyMCEPreInit?: PreInit };
...
const EditorManager: EditorManager = {
  ...
  /**
   * Major version of SSEEditor build.
   *
  ...
  /**
   * Minor version of SSEEditor build.
   *
  ...
  /**
   * Release date of SSEEditor build.
   *
   ...
   * @type sseeditor.Editor
   * @example
   * tinyMCE.activeEditor.selection.getContent();
   * sseeditor.EditorManager.activeEditor.selection.getContent();
   */
  activeEditor: null,
  focusedEditor: null,
  ...
  setup() {
    ...
    // If sseeditor is defined and has a base use that or use the old tinyMCEPreInit
    const preInit = window.sseeditor || window.tinyMCEPreInit;
    if (preInit) {
      baseURL = preInit.base || preInit.baseURL;
      suffix = preInit.suffix;
    } else {
      // Get base where the sseeditor script is located
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        // sseeditor.js sseeditor.min.js sseeditor.dev.js
        // sseeditor.jquery.js sseeditor.jquery.min.js sseeditor.jquery.dev.js
        // sseeditor.full.js sseeditor.full.min.js sseeditor.full.dev.js
        const srcScript = src.substring(src.lastIndexOf('/'));
        if (/sseeditor(\.full|\.jquery|)(\.min|\.dev|)\.js/.test(src)) {
    ...
    /**
     * Base URL where the root directory if SSEEditor is located.
     *
     * @property baseURL
     * @type String
     */
    ...
    /**
     * Absolute baseURI for the installation path of SSEEditor.
     *
     * @property baseURI
   ...
   * <br /><br />
   * Therefore, when using <code>overrideDefaults</code> with the cloud-based editor, any newly integrated options must be combined with the options in <code>sseeditor.defaultOptions</code>.
   *
   ...
   * };
   *
   * sseeditor.overrideDefaults({
   *   ...sseeditor.defaultOptions,
   *   ...customOptions
   * });
   */
   ...
   * <br /><br />
   * For information on basic usage of <code>init</code>, see: <a href="https://www.tiny.cloud/docs/sseeditor/6/basic-setup/">Basic setup</a>.
   *
   * @method init
    ...
    const findTargets = (options: RawEditorOptions): HTMLElement[] => {
      if (Env.browser.isIE() || Env.browser.isEdge()) {
        ErrorReporter.initError(
          'SSEEditor does not support the browser you are using. For a list of supported' +
          ' browsers please see: https://www.tiny.cloud/docs/sseeditor/6/support/#supportedwebbrowsers'
        );
        return [];
      } else if (isQuirksMode) {
        ErrorReporter.initError(
          'Failed to initialize the editor as the document is not in standards mode. ' +
          'SSEEditor requires standards mode.'
        );
        return [];
   ...
   * @return {sseeditor.Editor/Array} Editor instance or an array of editor instances.
   * @example
   * // Adds an onclick event to an editor by id
   * sseeditor.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by index
   * sseeditor.get(0).on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by id (longer version)
   * sseeditor.EditorManager.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   */
   ...
   *
   * @method add
   * @param {sseeditor.Editor} editor Editor instance to add to the collection.
   * @return {sseeditor.Editor} The same instance that got passed in.
   */
  add(editor) {
    const self: EditorManager = this;
   ...
   * @return {sseeditor.Editor} Editor instance that got created.
   */
   ...
   * sseeditor.remove('div');
   *
   * // Remove all editors bound to textareas
   * sseeditor.remove('textarea');
   *
   * // Remove all editors
   * sseeditor.remove();
   *
   ...
   *
   * @method remove
   * @param {sseeditor.Editor/String/Object} [selector] CSS selector or editor instance to remove.
   * @return {sseeditor.Editor} The editor that got passed in will be return if it was found otherwise null.
   */
   ...
   *
   * @method setActive
   * @param {sseeditor.Editor} editor Editor instance to set as the active instance.
   */
  setActive(editor) {
