...
import VisualBlocksPlugin from 'sseeditor/plugins/visualblocks/Plugin';

describe('browser.sseeditor.core.EditorCleanupTest', () => {
  before(() => {
    VisualBlocksPlugin();
  });
  ...
  const testCleanup = (comment: string, settings: RawEditorOptions, html: string = '<div></div>', fn: (editor: Editor) => void = Fun.noop) => {
    it(comment, async () => {
      ...
      const editor = await McEditor.pFromHtml<Editor>(html, { base_url: '/project/sseeditor/js/sseeditor', ...settings });
  ...
  testCleanup(
    ...
    '<div></div>',
    (editor) => {
      editor.dom.loadCSS('/project/sseeditor/js/sseeditor/skins/ui/dark/skin.css');
    });

  // Loaded via StyleSheetLoader
  assertPageLinkPresence('/project/sseeditor/js/sseeditor/skins/ui/content/default/content.inline.css', false);
  assertPageLinkPresence('/project/sseeditor/js/sseeditor/skins/ui/oxide/skin.css', false);
  // Loaded via DOMUtils as per above
  assertPageLinkPresence('/project/sseeditor/js/sseeditor/skins/ui/dark/skin.css', false);
});
