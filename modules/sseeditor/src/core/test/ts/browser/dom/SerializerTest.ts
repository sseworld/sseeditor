...
import DOMUtils from 'sseeditor/core/api/dom/DOMUtils';
import DomSerializer from 'sseeditor/core/api/dom/Serializer';
import * as TrimBody from 'sseeditor/core/dom/TrimBody';
...
describe('browser.sseeditor.core.dom.SerializerTest', () => {
  ...
  it('Schema rules', () => {
    ...
    setTestHtml('<img title="test" src="sseeditor/ui/img/raster.gif" data-mce-src="sseeditor/ui/img/raster.gif" alt="test" ' +
      'border="0" style="border: 1px solid red" class="test" /><span id="test2">test</span><hr />');
    assert.equal(
      ser.serialize(getTestElement(), { getInner: true }),
      '<img title="test" class="test" src="sseeditor/ui/img/raster.gif" ' +
    ...
    setTestHtml('<br /><hr /><input type="text" name="test" value="val" class="no" />' +
      ...
      'data-mce-href="file.html">link</a><a name="anchor"></a><a>no</a><img src="sseeditor/ui/img/raster.gif" ' +
      'data-mce-src="sseeditor/ui/img/raster.gif" />');
    assert.equal(ser.serialize(getTestElement()), '<div id="test"><br><hr><input type="text" name="test" value="val">' +
      ...
      '<a name="anchor"></a>no<img src="sseeditor/ui/img/raster.gif" border="0" title="mce_0"></div>', 'Output name and attribute rules');

    ser.setRules('img[src|border=0|alt=]');
    setTestHtml('<img src="sseeditor/ui/img/raster.gif" data-mce-src="sseeditor/ui/img/raster.gif" border="0" alt="" />');
    assert.equal(ser.serialize(getTestElement()), '<img src="sseeditor/ui/img/raster.gif" border="0" alt="">', 'Default attribute with empty value');

    ser.setRules('img[src|border=0|alt=],div[style|id],*[*]');
    setTestHtml('<img src="sseeditor/ui/img/raster.gif" data-mce-src="sseeditor/ui/img/raster.gif" /><hr />');
    assert.equal(
      ser.serialize(getTestElement(), { getInner: true }),
      '<img src="sseeditor/ui/img/raster.gif" border="0" alt=""><hr>'
    );
    ...
    setTestHtml('<img src="sseeditor/ui/img/raster.gif" data-mce-src="sseeditor/ui/img/raster.gif" alt="" />');
    assert.equal(
      ser.serialize(getTestElement()),
      '<div id="test"><img src="sseeditor/ui/img/raster.gif" alt=""></div>'
    );

    ser = DomSerializer({ invalid_elements: 'hr,br' });
    setTestHtml('<img src="sseeditor/ui/img/raster.gif" data-mce-src="sseeditor/ui/img/raster.gif" /><hr /><br />');
    assert.equal(
      ser.serialize(getTestElement(), { getInner: true }),
      '<img src="sseeditor/ui/img/raster.gif">'
    );
  });
