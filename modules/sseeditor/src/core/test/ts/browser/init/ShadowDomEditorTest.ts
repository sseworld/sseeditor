...
describe('browser.sseeditor.core.init.ShadowDomEditorTest', () => {
  before(function () {
  ...
  Arr.each([
    ...
    context(`${tester.type} editor`, () => {
      ...
      it('aux div should be within shadow root', async () => {
        ...
        assert.lengthOf(SelectorFilter.descendants(SugarBody.body(), '.tox-sseeditor-aux'), 0, 'Should be no aux divs in the document');
        assert.lengthOf(
          SelectorFilter.descendants(shadowRoot, '.tox-sseeditor-aux'),
          tester.numSinks,
