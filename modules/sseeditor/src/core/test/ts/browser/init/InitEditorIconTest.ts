...
import { getAll as getDefaultIcons } from '@sseeditor/oxide-icons-default';
import { assert } from 'chai';

import Editor from 'sseeditor/core/api/Editor';
import IconManager from 'sseeditor/core/api/IconManager';

describe('browser.sseeditor.core.init.InitEditorIconTest', () => {
  const overrideIcon = '<svg>override-icon</svg>';
  const hook = TinyHooks.bddSetupLight<Editor>({
    icons: 'custom',
    icons_url: '/project/sseeditor/src/core/test/assets/icons/custom/icons.js',
