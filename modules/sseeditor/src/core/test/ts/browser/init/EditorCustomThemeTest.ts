...
import { Theme } from 'sseeditor/core/api/ThemeManager';
import { SSEEditor } from 'sseeditor/core/api/SSEEditor';

declare const sseeditor: SSEEditor;

describe('browser.sseeditor.core.init.EditorCustomThemeTest', () => {
  ...
  context('named synchronous custom theme', () => {
    ...
    const SyncTheme = () => {
      sseeditor.ThemeManager.add('sync_theme', (editor): Theme => {
        const theme: Theme = {
          renderUI: () => {
            const editorContainer = document.createElement('div');
