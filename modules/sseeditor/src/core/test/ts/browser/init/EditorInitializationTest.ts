...
describe('browser.sseeditor.core.init.EditorInitializationTest', () => {
  ...
  before(() => {
    EditorManager._setBaseUrl('/project/sseeditor/js/sseeditor');
  ...
  it('suffix option is used by the skin', async () => {
    viewBlock.update('<div class="sseeditor-editor"><p>a</p></div>');
    await EditorManager.init({
      selector: '.sseeditor-editor',
      inline: true,
      promotion: false,
      toolbar_mode: 'wrap',
      suffix: '.x'
  ...
  const pCreateInlineModeMultipleInstances = (): Promise<Editor[]> => {
    viewBlock.update('<div class="sseeditor-editor"><p>a</p></div><div class="sseeditor-editor"><p>b</p></div>');
  ...
  const assertEditors = (editors: Editor[]) => {
    ...
    const containerApproxStructure = ApproxStructure.build((s, str, arr) => s.element('div', {
      classes: [ arr.has('tox'), arr.has('tox-sseeditor'), arr.has('tox-sseeditor-inline') ],
      children: [
        s.element('div', {
