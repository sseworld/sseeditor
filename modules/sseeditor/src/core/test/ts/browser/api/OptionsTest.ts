...
import * as Options from 'sseeditor/core/api/Options';

describe('browser.sseeditor.core.api.OptionsTest', () => {
  it('getBodyId', () => {
    assert.equal(Options.getBodyId(new Editor('id', {}, EditorManager)), 'sseeditor', 'Should be default id');
    ...
    assert.equal(Options.getBodyId(new Editor('idc', { body_id: 'ida=a,idb=b' }, EditorManager)), 'sseeditor', 'Should be default id for idc');
  });
