...
import StyleSheetLoader from 'sseeditor/core/api/dom/StyleSheetLoader';
import Editor from 'sseeditor/core/api/Editor';
import { SSEEditor } from 'sseeditor/core/api/SSEEditor';
...
const loadUiSkins = async (editor: Editor, skinUrl: string): Promise<void> => {
  ...
  const css = sseeditor.Resource.get(skinUiCss);
...
const loadShadowDomUiSkins = async (editor: Editor, skinUrl: string): Promise<void> => {
  ...
  if (isInShadowRoot) {
    ...
    const css = sseeditor.Resource.get(shadowDomSkinCss);
...
const loadUrlSkin = async (isInline: boolean, editor: Editor): Promise<void> => {
  ...
  Options.getSkinUrlOption(editor).fold(unbundled, (skinUrl) => {
    ...
    const css = sseeditor.Resource.get(skinContentCss);
    if (Type.isString(css)) {
