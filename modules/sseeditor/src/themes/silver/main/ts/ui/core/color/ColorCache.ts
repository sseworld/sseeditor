...
import { Menu } from 'sseeditor/core/api/ui/Ui';
import LocalStorage from 'sseeditor/core/api/util/LocalStorage';
...
const getCacheForId = (id: string): ColorCache =>
  Obj.get(cacheStorage, id).getOrThunk(() => {
    const storageId = `sseeditor-custom-colors-${id}`;
    const currentData = LocalStorage.getItem(storageId);

    if (Type.isNullable(currentData)) {
      const legacyDefault = LocalStorage.getItem('sseeditor-custom-colors');
