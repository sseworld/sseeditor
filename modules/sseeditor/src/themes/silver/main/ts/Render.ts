...
import { EditorUiApi } from 'sseeditor/core/api/ui/Ui';
...
const setup = (editor: Editor, setupForTheme: ThemeRenderSetup): RenderInfo => {
  ...
  const renderDialogUi = () => {
    ...
    const sinkSpec = {
      dom: {
        tag: 'div',
        classes: [ 'tox', 'tox-silver-sink', 'tox-sseeditor-aux' ].concat(deviceClasses),
        attributes: {
          ...I18n.isRtl() ? { dir: 'rtl' } : {}
        }
      },
  ...
  const renderPopupUi = () => {
    ...
    const sinkSpec = {
      dom: {
        tag: 'div',
        classes: [ 'tox', 'tox-silver-sink', 'tox-silver-popup-sink', 'tox-sseeditor-aux' ].concat(deviceClasses),
        attributes: {
          ...I18n.isRtl() ? { dir: 'rtl' } : {}
        }
      },
  ...
  const renderMainUi = () => {
    ...
    const outerContainer = GuiFactory.build(
      OuterContainer.sketch({
        dom: {
          tag: 'div',
          classes: [ 'tox', 'tox-sseeditor' ]
            .concat(isInline ? [ 'tox-sseeditor-inline' ] : [])
            .concat(isToolbarBottom ? [ 'tox-sseeditor--toolbar-bottom' ] : [])
        ...
        behaviours: Behaviour.derive([
          ...
          Disabling.config({
            disableClass: 'tox-sseeditor--disabled'
          }),
          Keying.config({
            mode: 'cyclic',
