...
const resourceBase = 'sseeditor.html-i18n';
...
const createSSEEditorJs = (translation, resourceName) =>
  `sseeditor.Resource.add('${resourceName}',\n${util.inspect(translation)});`
...
const generate = (grunt, srcDir, outputDir, resourcePrefix) => {
  findFiles(srcDir).forEach((file) => {
    ...
    const content = createSSEEditorJs(translation, `${resourcePrefix}.${langCode}`);
