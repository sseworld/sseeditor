    ...
    plugins: [
      ...
      new webpack.WatchIgnorePlugin({
        paths: [
          ...
          /sseeditor\/modules\/.*\/lib\/.*\.(js|map|d\.ts)$/,
...
const buildDemoEntries = (typeNames, type, demo, pathPrefix = "") =>
...
const buildEntries = (typeNames, type, entry, pathPrefix = "") =>
  typeNames.reduce((acc, name) => {
    const fileName = type.replace(/s$/, "") + ".js";
    acc[
      `${pathPrefix}${type}/${name}/${fileName}`
    ] = `src/${type}/${name}/main/ts/${entry}`;
    return acc;
  }, {});
...
const all = (plugins, themes, models) => {
  return [
    create(
      {
        ...
        "scratch/demos/core/cspdemo.js":
          "src/core/demo/ts/demo/ContentSecurityPolicyDemo.ts",
        ...buildDemoEntries(plugins, "plugins", "Demo.ts", "scratch/demos/"),
        ...buildEntries(plugins, "plugins", "Main.ts", "js/sseeditor/"),
        ...buildDemoEntries(models, "models", "Demo.ts", "scratch/demos/"),
        ...buildEntries(models, "models", "Main.ts", "js/sseeditor/"),
        ...buildDemoEntries(themes, "themes", "Demos.ts", "scratch/demos/"),
        ...buildEntries(themes, "themes", "Main.ts", "js/sseeditor/"),
      },
      "../../tsconfig.demo.json"
    ),
    ...
    create(
      {
        "js/sseeditor/sseeditor.js": "src/core/main/ts/api/Main.ts",
      },
      "../../tsconfig.json"
    ),
  ];
...
const generateDemoIndex = (grunt, app, plugins, themes, models) => {
  const demoList = grunt.file.expand([
    "src/**/demo/html/*.html",
    "src/**/demo/html/**/*.html",
  ]);
  ...
  const lists = Object.keys(sortedDemos)
    .map(
      (type) => `
    <h2>${type}</h2>
    <ul>
      ${sortedDemos[type]
        .map(
          (link) =>
            `<li>${
              type !== "core" ? `<strong>${link.split("/")[2]}</strong> - ` : ""
            }<a href="${link}">${path.basename(link)}</a></li>`
        )
        .join("")}
    </ul>`
    )
